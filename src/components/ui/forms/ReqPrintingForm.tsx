...
//Form for order a printing
export default function RequirePrintingForm() {
  const [state, formAction, isPending] = useActionState(
    OrderAction,
    initialState
  );
  const [functionalityOption, setFunctionalityOption] = useState('option1');
  const [materialOption, setMaterialOption] = useState('PLA');
  const [resetTrigger, setResetTrigger] = useState<boolean>(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [modalMessage, setModalMessage] = useState('');
  const [fileForPreview, setFileForPreview] = useState<File[]>([]);
  const handleFuntionalityRadioChange = (event: {
    target: { value: SetStateAction<string> };
  }) => {
    setFunctionalityOption(event.target.value);
  };

  const handleMaterialRadioChange = (event: {
    target: { value: SetStateAction<string> };
  }) => {
    setMaterialOption(event.target.value);
  };
    ...
    <>
      ...
      <div className="my-5 flex flex-col gap-2">
        <h1 className="text-4xl font-bold text-center text-white">
          Request Printing Form
        </h1>
        <p className="text-sm text-center text-[#f1cca3]">
          Need a custom 3D printing? Fill out the form below with your
          requirements.
        </p>
      </div>
      <form
        ...
        <div className="mx-5">
          ...
          <section className="mb-4">
            ...
            <Input
              ...
              id="phone"
              name="phone"
              placeholder="Phone Number (WhatsApp Purpose)"
              autoComplete="true"
              required
              className=" text-white border-gray-600"
            />
          ...
          <section className="flex flex-col gap-3 mb-4">
            <FileUpload
              onChange={setFileForPreview}
              resetTrigger={resetTrigger}
              maxSizeMB={100}
              allowedExtensions={['.zip', '.stl']}
            />
            <STLViewer file={fileForPreview[0]} />
            ...
            <div className="flex gap-5 my24">
              ...
              <div className="flex flex-col gap-1 items-center">
                <input
                  ...
                  disabled
                  name="material"
                  className="w-10 h-6 accent-[#9c200d] cursor-not-allowed"
                  checked={materialOption === 'ABS'}
                  onChange={handleMaterialRadioChange}
                />
                <Label
                  htmlFor="material-pla"
            ...
            <div className="flex gap-5 my24">
              <div className="flex flex-col gap-1 items-center">
                <input
                  ...
                  onChange={handleFuntionalityRadioChange}
                />
                <Label
